  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      for (const design of designs) {
        ...
        // 1. Replace QR Code placeholder in the first image tag
        svgText = svgText.replace(/(<image[^>]*href=")[^"]*(")/, `$1${qrCodeDataUrl}$2`);
        svgText = svgText.replace(/(<image[^>]*xlink:href=")[^"]*(")/, `$1${qrCodeDataUrl}$2`);
